...
DIR_TEMP = r'C:\Users\zhudw\Desktop\android\ad_temp_3.txt'  # 目录
...
class Handler(BaseHandler):
    ...
    def get_links(self, response):
        ...
        self.crawl(final_url, callback=self.get_result, fetch_type='js', headers=self.heard)
    ...
    def get_result(self, response):
        ...
        self.crawl(final_url, callback=self.index_page, fetch_type='js', headers=self.heard)
    ...
    def detail_page(self, response):
        ...
        aws = [x.text() for x in response.doc('.result-answer-item pre').items()]
        s = ""
        s += title + "\n" + right_aw + "\n"
        for a in aws:
            s += a + "\n"
        ...
        return dict(title=title, right_aw=right_aw, aws=aws)
