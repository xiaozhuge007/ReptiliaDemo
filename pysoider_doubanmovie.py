...
class Handler(BaseHandler):
    ...
    @every(minutes=24 * 60)
    def on_start(self):
        self.crawl('https://movie.douban.com/tag/?view=cloud', callback=self.index_page, headers=gen_headers(),
                   cookies=random_bid())
    ...
    def index_page(self, response):
        for each in response.doc(
                '#content > div > div.article > div.indent.tag_cloud > table > tbody > tr > td > a').items():
    ...
    def movie_list_page(self, response):
        ...
        for movie_link in response.doc('#content div.pl2 > a').items():
            self.crawl(movie_link.attr.href, callback=self.movie_detail_page, headers=gen_headers(),
                       cookies=random_bid())
    ...
    def movie_detail_page(self, response):
        ...
        return {
            ...
            'rate': str_to_float(
                response.doc('#interest_sectl > div.rating_wrap.clearbox > div.rating_self.clearfix > strong').text()),
            'screenwriter': [name.strip() for name in
                             response.doc('#info > span:nth-child(3) > span.attrs').text().split('/')],
            ...
            'state': [country.strip() for country in
                      response.etree.xpath('//*[@id="info"]/span[text()="制片国家/地区:"]')[0].tail.split('/')],
            'language': response.etree.xpath('//*[@id="info"]/span[text()="语言:"]')[0].tail,
            'release_date': self.trim_suffix(
                response.etree.xpath('//*[@id="info"]/span[@property="v:initialReleaseDate"]/text()')[0]),
